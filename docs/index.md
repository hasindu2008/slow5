# SLOW5 Ecosystem

Here we keeps track of all the links/pages associated with SLOW5 ecosystem in a single place. Select a category:

- [SLOW5 specification](https://hasindu2008.github.io/slow5specs)
- [Software libraries](libs.md)
- [Bioinformatic tools](https://hasindu2008.github.io/slow5tools/software.html)
- [Bioinformatics workflows](workflows.md)
- [Open datasets](data.md)
- [Learning resourses](learn.md)
- [Publications](pub.md)
- [Code repositories](repo.md)

If you are very new to nanopore signal data in relation to the SLOW5 file format, a getting started tutorial is now available [here](tutorial.md).

---

The core publication for the SLOW5 ecosystem:
> Gamaarachchi, H., Samarakoon, H., Jenner, S.P. et al. Fast nanopore sequencing data analysis with SLOW5. Nat Biotechnol 40, 1026-1029 (2022). [https://doi.org/10.1038/s41587-021-01147-4](https://doi.org/10.1038/s41587-021-01147-4)
